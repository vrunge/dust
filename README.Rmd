...
We generate some 1D time series of length `400` from the Gaussian model and two changes in the sequence.
...
The object is empty. We append data to this object using `append_c` and run the dynamic programming algorithm with `update_partition()`. The current partition is returned using a backtracking algorithm with `get_partition()`. 
